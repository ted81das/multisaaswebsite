...
class MenuBuilderAdminRender
{
    ...
    private function get_draggable_fields_markup(array $attributes_string,$lang)
    {
        ...
        if (!empty($matches[0])) {
            ...
            $output .= '<input type="text" class="static_pname" placeholder="eg: menu label" value="'.$attributes_string['pname'].'">';
        }else{
                ...
                if ($dynamic_menu_type){
                    ...
                    if ($dynamic_menu_type['query'] === 'old_lang'){
                        ...
                        if (!is_null($item_details)){
                            $title = $item_details->getTranslation('title',get_user_lang());
                        }
                    }
