...
Route::middleware(['maintenance_mode','landlord_glvar'])->controller(PaymentLogController::class)->name('landlord.')->group(function () {
    ...
    Route::post('/paymob-ipn', 'paymob_ipn')->name('frontend.paymob.ipn');
...
Route::middleware(['landlord_glvar','maintenance_mode','setlang','Google2FA'])->controller(\App\Http\Controllers\Landlord\Frontend\LandlordFrontendController::class)->name('landlord.')->group(function () {
...
Route::get("assets/payment-gateway/screenshot/{moduleName}/{gatewayName}", function ($moduleName, $gatewayName){
    ...
    return abort(404);
})->middleware('setlang')->name("payment.gateway.logo");
