...
Route::middleware([
    ...
    Route::middleware('package_expire')->controller(\App\Http\Controllers\Landlord\Admin\Auth\AdminLoginController::class)->prefix('admin')->group(function (){
        ...
        Route::post('/login/reset-password','TenantResetPassword')->name('tenant.reset.password.change');
    });
});
...
Route::middleware(['maintenance_mode'])->controller(\App\Http\Controllers\Tenant\Frontend\PaymentLogController::class)->name('tenant.')->group(function () {
    ...
    Route::controller(\App\Http\Controllers\Tenant\Frontend\UserDashboardController::class)->middleware(['tenantUserMailVerify','package_expire','setlang'])->name('tenant.')->group(function(){
        ...
        Route::name('user.dashboard.')->group(function()
        {
            Route::post('/cancel-order-request/{id}', 'order_cancle_request')->name('order.cancel.request');
            Route::get('/user-menu-orders', 'user_all_menu_order')->name('user.order');
            Route::get('/restaurant-orders', 'restaurant_order')->name('restaurant-order');
            Route::get('/view-orders/{id}', 'view_order')->name('view.order');
            Route::get('/user-canceled-orders', 'all_user_canceled_order')->name('canceled.order');
            Route::get('/user-pending-orders', 'all_user_pending_order')->name('pending.order');
            Route::get('/user-approved-orders', 'all_user_approved_orders')->name('approved.order');
        });
