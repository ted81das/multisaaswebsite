...
@section('scripts')
    <script>
        $(function (){
            ...
            //kineticpay_bank
            $(document).on('click', '#kineticpay_bank', function() {
                $('.checkout-form input[name="kineticpay_bank"]').val( $(this).val());
            });
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        ...
                        if (data.type == 'success')
                        {
                            ...
                            $('.coupon-amount-span').text(data.coupon_amount);
            ...
            $(document).on('click', '.payment-gateway-wrapper > ul > li', function (e) {
                ...
                let gateway_el = $(this);
                let gateway_data = gateway_el.data('gateway');
                gateway_el.addClass('selected').siblings().removeClass('selected');
                $('.payment_gateway_passing_clicking_name').val(gateway_data);

            });

            $(document).on('keyup', '.manual_transaction_id input', function (e){
                $('input[name=transaction_id]').val($(this).val());
            });

            // when change bank transfer file value will be added with bank_transfer_attachment

            $(document).on('change', '.bank_transfer', function (e) {
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        ...
                        // Set the base64 encoded file content into the hidden input field
                        $('#manual_payment_attachment').val(fileContent);
                    };
