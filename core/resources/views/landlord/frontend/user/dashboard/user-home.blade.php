                    ...
                    <table class="table table-responsive table-bordered recent_payment_table">
                        ...
                        @foreach($user->tenant_details ?? [] as $key => $data)
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="#" class="badge rounded-pill bg-primary px-4"  data-subdomain="{{$data->id}}" data-bs-toggle="modal" id="upgrade_plan" data-bs-target="#upgrade_website_modal">{{__('Upgrade Plan')}}</a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group payment_parent">
                            ...
                            <select name="selected_payment_gateway" class="form-control payment_gateway">
                                ...
                                @foreach($all_gateway_with_custom as $payment)
                                    @if($payment['name'] === "iyzipay")
                                        @if(get_static_option('iyzipay_payment_gateway_status') == null)
                                            @continue;
                                        @endif
                                    @endif
...
{{--    plan upgrade modal --}}
    <div class="modal fade" id="upgrade_website_modal" aria-hidden="true">
        <div class="modal-dialog ">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{__('Upgrade Website')}}</h5>
                    <button type="button" class="close bg-danger text-white" data-bs-dismiss="modal"><span>×</span></button>
                </div>
                <form action="{{route('landlord.frontend.order.payment.form')}}" method="post" enctype="multipart/form-data" class="register_page_payment_hidden_form" >
                    @csrf
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group domain_area">
                            <strong for="name">{{__('Your Domain')}}</strong>
                            <input type="text" class="form-control" name="subdomain" placeholder="Your domain" readonly>
                        </div>
