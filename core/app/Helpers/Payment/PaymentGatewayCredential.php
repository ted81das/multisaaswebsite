...
class PaymentGatewayCredential
{
    ...
    public static function get_paymob_credential(): object
    {
        $paymob_credential_from_database = PaymentGateway::where('name', 'paymob')->first();
        $decoded = json_decode($paypal_credential_from_database->credentials) ?? [];
        $test_mode = $paymob_credential_from_database->test_mode;

        $api_key = $decoded->api_key ?? '';
        $integration_id = $decoded->integration_id ?? '';
        $iframe_id = $decoded->iframe_id ?? '';
        $secret_key = $decoded->secret_key ?? '';
        $public_key = $decoded->public_key ?? '';
        $hmac_secret = $decoded->hmac_secret ?? '';

        $paymob = XgPaymentGateway::paymob();

        $paymob->setApiKey($api_key);
        $paymob->setIntegrationId($integration_id);
        $paymob->setIframeId($iframe_id);
        $paymob->setGatewayType('accept-online');

        $paymob->setSecretKey($secret_key);
        $paymob->setPublicKey($public_key);
        $paymob->setHmacSecret($hmac_secret);
        $paymob->setEnv($test_mode);
        $paymob->setCurrency(self::site_global_currency());
    ...
    public static function get_awdpay_credential(): object
    {
        ...
        $test_mode = $awdpay_credential_from_database->test_mode;

        $private_key = $decoded->private_key ?? '';

        $awdpay = XgPaymentGateway::awdpay();
        $awdpay->setPrivateKey($private_key);
        $awdpay->setEnv($test_mode);
        $awdpay->setCurrency(self::site_global_currency());

        return $awdpay;
    }

    public static function get_powertranzpay_credential(): object
    {
        $powertranzpay_credential_from_database = PaymentGateway::where('name', 'powertranzpay')->first();

        $decoded = json_decode($powertranzpay_credential_from_database->credentials) ?? [];
        $test_mode = $powertranzpay_credential_from_database->test_mode;

        $private_key = $decoded->private_key ?? '';

        $powertranzpay = XgPaymentGateway::powertranzpay();
        $powertranzpay->setPrivateKey($private_key);
        $powertranzpay->setEnv($test_mode);
        $powertranzpay->setCurrency(self::site_global_currency());

        return $powertranzpay;
    }
