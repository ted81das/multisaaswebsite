...
{
    try {
        return StaticOptionCentral::where('option_name', $key)->delete();
    } catch (\Exception $e) {
        //handle error
    }
}
...
function render_payment_gateway_for_form($cash_on_delivery = false, $exclude = [])
{
    // If the custom payment gateway not active exclude it
    $exclude = empty(get_static_option('iyzipay_payment_gateway_status')) ? ['iyzipay'] : [];
    ...
    $all_gateway = \App\Models\PaymentGateway::active()->get();
...
//        if ($gateway['status'] == true) {
        if (in_array($gateway['name'], $exclude)) {
            continue;
        }
...
//                    </div>';
    ...
    $output .= ' <div class="form-group bank_payment_field ' . $conditon_2 . '">
                            ...
                            <input class="form-control btn-sm mb-3 py-3 p-3 bank_transfer" type="file" name="manual_payment_attachment">
