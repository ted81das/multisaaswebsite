...
class TenantFrontendController extends Controller
{
    ...
    public function dynamic_single_page ($slug){
        ...
        if ($slug === $shop_page_slug) {
            if (tenant()) {
                ...
                $sizes = Size::whereHas('product_sizes',function ($query){
                    $query->whereHas('products',function ($q){
                        $q->whereNull('deleted_at');
                    });
                })->select('id', 'name', 'size_code', 'slug')->get();

                $colors = Color::whereHas('product_colors',function ($query){
                    $query->whereHas('products',function ($q){
                        $q->whereNull('deleted_at');
                    });
                })->select('id', 'name', 'color_code', 'slug')->get();


                $tags = ProductTag::whereHas('product',function ($query)
                {
                    $query->whereNull('deleted_at');
                })->select('tag_name')->distinct()->get();
    ...
    {
        $redirect_url = request()->query('return');
        $redirect_url_session = session('redirect_url');

        if (auth('web')->check())
        {
            if ($redirect_url || $redirect_url_session)
            {
                return redirect(url('/') . $redirect_url_session);
            }
        ...
        if($redirect_url)
        {
            Session::flash('message', 'Please Login first to Confirm your order');
        }
