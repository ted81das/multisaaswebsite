...
class PaymentSettingsController extends Controller
{
    ...
    public function paymob_settings()
    {
        $gateway = PaymentGateway::where('name','paymob')->first();
        if(is_null($gateway)){
            $this->seedPaymentGatewayData('paymob');
        }
        return view(self::BASE_PATH.'payment-data',compact('gateway'));
    }

    public function awdpay_settings()
    {
        $gateway = PaymentGateway::where('name','awdpay')->first();
        if(is_null($gateway)){
            $this->seedPaymentGatewayData('awdpay');
        }
        return view(self::BASE_PATH.'payment-data',compact('gateway'));
    }

    public function powertranzpay_settings()
    {
        $gateway = PaymentGateway::where('name','powertranzpay')->first();
        if(is_null($gateway)){
            $this->seedPaymentGatewayData('powertranzpay');
        }
        return view(self::BASE_PATH.'payment-data',compact('gateway'));
    }
    ...
    public function iyzipay_settings()
    {
        ...
        if(is_null($gateway)){
            $this->seedPaymentGatewayData('iyzipay');
        }
        return view(self::BASE_PATH.'payment-data',compact('gateway'));
    }
