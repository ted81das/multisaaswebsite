...
class PaymentGatewayIpn
{
    ...
    public function send_order_mail($order_id)
    {
        ...
        try {
            ...
            {
                Mail::to($order_details->email)->send(new ProductOrderEmail($order_details));
            } else {
                Mail::to($order_details->email)->send(new ProductOrderManualEmail($order_details));
            }

            // To Admin
            $admin_email = get_static_option('order_receiving_email') ?? get_static_option('site_global_email');
            if ($admin_email == null)
            {
