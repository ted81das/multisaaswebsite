...
class DatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        // removing duplicate payment gateways from database
        $this->removeDuplicatePaymentGateway();
        ...
        //For new tenant permission store
        $this->tenant_new_permission_seed();
    ...
    private function removeDuplicatePaymentGateway()
    {
        $payment_gateways = PaymentGateway::where('name', 'bank_transfer')->skip(1)->take(20)->orderBy('id')->get();

        try {
            for ($i = 0; $i < count($payment_gateways); $i++) {
                $payment_gateways[$i]->delete();
            }
