<div class="col-xl-5 col-lg-5 mt-4">
    <div class="checkout-order-summery border-1">
        <div class="order-summery-contents">
            ...
            <form action="#" class="coupon-contents-form coupon-border pt-4 mt-4" enctype="multipart/form-data">
                <div class="single-input">
                    ...
                    <input type="hidden" class="coupon-shipping-method" name="coupon_shipping_method"
                           value="">
                    <input class="form--control coupon-code" type="text" placeholder="{{__('Coupon Code')}}" name="used_coupon">
                </div>
                        ...
                        <div class="single-check-carts">
                            <div class="check-cart-flex-contents">
                                ...
                                <div class="checkout-cart-img-contents">
                                    <h6 class="checkout-cart-title"><a href="javascript:void(0)"> {{$data->name}} </a></h6>
                                    <span class="product-items"> {{__('Quantity:')}} {{$data->qty}} </span>
                            ...
                            <span class="checkout-cart-price color-heading fw-500"> {{amount_with_currency_symbol($data->price * $data->qty)}} </span>
                        </div>
                    </div>
                @endforeach
            </div>
                ...
                <ul class="coupon-contents-details-list coupon-border">
                    <li class="coupon-contents-details-list-item">
                        <h6 class="coupon-contents-details-list-item-title"> {{__('Sub Total')}} </h6> <span
                            class="coupon-contents-details-list-item-price fw-500"> {{site_currency_symbol().\Gloudemans\Shoppingcart\Facades\Cart::subtotal()}} </span>
                    </li>
                </ul>
                <div class="shipping_method_wrapper">
                    ...
                    <div class="shipping_tax_total">
                        <ul class="coupon-contents-details-list coupon-border">
                            ...
                            <li class="coupon-contents-details-list-item coupon-discount"><span> {{__('Coupon Discount (-)')}} </span>
                                <span class="coupon-amount-span coupon-amount-span"> {{ amount_with_currency_symbol(0.00) }} </span></li>
                        ...
                        <ul class="coupon-contents-details-list coupon-border">
                            ...
                            ">
                            <h6 class="coupon-title"> {{__('Total Amount')}} </h6>
                            <span class="coupon-price fw-500 color-heading"> {{site_currency_symbol().$total}} </span>
            ...
            <div class="payment-inlines mt-4">
                ...
                <div class="payment-card mt-4">
                    ...
                    <input type="hidden" name="kineticpay_bank" value="{{get_static_option('site_default_payment_gateway')}}"
                           class="payment_bank_name">
            ...
            <div class="checkbox-inlines mt-3">
                ...
                <label class="checkbox-label" for="agree"> {{__('I have read and agree to the website')}} <a
                        class="terms-condition" href="{{route('tenant.dynamic.page',$terms_page_route)}}"> {{__('terms and conditions*')}} </a> </label>
            </div>

            <div class="btn-wrapper mt-3">
                <a href="javascript:void(0)"
                   class="cmn-btn cmn-btn-outline-two w-100 radius-0 checkout_disable proceed_checkout_btn"> {{__('Proceed to
                    Checkout')}} </a>
            </div>

            <div class="btn-wrapper mt-3">
                <a href="{{route('tenant.shop.cart')}}" class="cmn-btn-outline3 w-100 radius-0"> {{__('Return to Cart')}} </a>
            </div>
        </div>
    </div>
</div>
