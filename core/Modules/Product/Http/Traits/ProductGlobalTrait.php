...
trait ProductGlobalTrait {
    ...
    public function productUpdate($data, $id) {
        ...
        if(!empty($data["item_stock_count"][0])){
            ...
            $productAttribute = $this->deleteProductAttribute($id,$product?->inventory?->id);
        }
    ...
    public function deleteProductAttribute($product_id, $inventory_id)
    {
        // Retrieve the IDs of product inventory details matching the given product and inventory
        $prd_in_detail = ProductInventoryDetail::where([
            "product_id" => $product_id,
            "product_inventory_id" => $inventory_id
        ])->pluck("id")->toArray();

        if (!empty($prd_in_detail)) {
            // Delete the product inventory detail attributes associated with the retrieved inventory detail IDs
            ProductInventoryDetailAttribute::where("product_id", $product_id)
                ->whereIn("inventory_details_id", $prd_in_detail)
                ->delete();

            // Delete the product inventory details
            ProductInventoryDetail::where([
                "product_id" => $product_id,
                "product_inventory_id" => $inventory_id
            ])->delete();
